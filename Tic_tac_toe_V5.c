...
void train_neural_network(struct fann *ann, int moves[], int num_moves);
...
void train_neural_network(struct fann *ann, int moves[], int num_moves)
{
    ...
    for (int i = 0; i < num_moves; ++i)
    {
        ...
        for (int j = 0; j < 9; ++j)
        {
            input[j] = (fann_type)moves[i * 18 + j];
            output[j] = (fann_type)moves[i * 18 + 9 + j];
        }

        // Add the pattern to the training data set
        fann_train_push(data, input, output);
    }
...
int neural_network_move(struct fann *ann, Tic_tac_toe *game)
{
    ...
    if (num_empty_cells > 0)
    {
        ...
        int predicted_move = 1 + (int)fann_run(ann, (fann_type*)moves);
